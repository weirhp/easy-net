...
inline bool ShouldInjectImpl(const Character* command_line,
    ...
    // can prevent the renderer from starting. Only its unsandboxed network
    // service needs the Winsock hook; ordinary non-Chromium children retain the
    // previous recursive-injection behavior.
