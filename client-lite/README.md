...
新建配置默认同时勾选“中国大陆 IP 直连（国外 IP 走代理）”。SOCKS5 TCP、HTTP CONNECT 和 UDP 目标会使用内置的 APNIC 中国大陆 IPv4/IPv6 地址表分流：字面 IP 命中地址表时走本机网络，未命中时走配置的 WebSocket/SSH 代理。公网域名不会再交给 Windows DNS 判断国内外；Lite 会通过当前加密代理通道访问可信 DoH，只有安全解析结果全部属于中国大陆时才固定该地址直连，解析失败、混合结果或非大陆结果都会保留原域名并交给代理端解析。局域网短名称和 `.local`、`.lan`、`.internal` 等本地域名仍可使用系统 DNS。解析结果有短时缓存，不会为每条连接重复查询。修改正在运行的配置后需要停止再启动该配置才会生效。
...
- Chrome/Edge 默认使用 Chromium 原生 SOCKS5 参数启动，并阻止本机 DNS 预解析；这是本机 DNS 可能受污染时的推荐方式。建议保留“独立代理配置目录”，否则已经运行的浏览器可能接收新窗口并忽略本次代理参数。
