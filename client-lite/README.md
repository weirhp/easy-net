...
勾选“中国大陆 IP 直连（国外 IP 走代理）”后，SOCKS5 TCP、HTTP CONNECT 和 UDP 目标会使用内置的 APNIC 中国大陆 IPv4/IPv6 地址表分流。IP 命中地址表时走本机网络，未命中时走配置的 WebSocket/SSH 代理。域名会先用本机 DNS 解析，只有全部解析结果都命中已启用的直连规则时才直连，避免混合解析结果泄漏。这个功能按 IP 而不是域名分类，CDN 和跨境服务的实际线路取决于本机 DNS 返回的地址。修改正在运行的配置后需要停止再启动该配置才会生效。
